# Hybrid proof contract — v3

Frozen by Mike Wilson’s owner review and authorization on 2026-09-08. Public implementation: `sdk/node/pqc/` and `sdk/python/vella/pqc/`. The private draft and feasibility formats are not aliases for v3.
...
- Payload type: `application/vnd.vella.authorization.v3+json`.
- Signature-policy type: `application/vnd.vella.signature-policy.v3+json`.
- Suite: `p256+ml-dsa-65`; this version has no PQ-only accepted profile.
...
`payload_hash` is SHA-384 of this complete message, prefixed `sha384:`. This is an application-specific hybrid envelope, not the DSSE envelope schema or an IETF Composite ML-DSA encoding. Composition accepted in the owner contract review; this is not external cryptographic certification.
