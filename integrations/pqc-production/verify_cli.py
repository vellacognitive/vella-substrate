...
try:
    ...
    if type(trust) is not dict or set(trust) != {'kind', 'revision', 'activeId', 'history'} or trust['kind'] != 'vella_local_hybrid_keys_draft1' or not integer(trust['revision']) or trust['revision'] < 0 or type(trust['history']) is not list or len(trust['history']) > 256:
