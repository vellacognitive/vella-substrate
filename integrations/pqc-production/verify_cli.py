"""Private review CLI, with explicit historical profile and operator trust."""
import json
import os
import stat
import sys
...
def read(path, maximum, trusted=False):
...
try:
    ...
    if type(trust) is not dict or set(trust) != {'kind', 'revision', 'activeId', 'history'} or trust['kind'] != 'vella_local_hybrid_keys_draft1' or type(trust['revision']) is not int or trust['revision'] < 0 or type(trust['history']) is not list or len(trust['history']) > 256:
