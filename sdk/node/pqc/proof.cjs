...
const KIND = 'vella_proof_bundle_v3';
const TYPE = 'application/vnd.vella.authorization.v3+json';
const CONTEXT = 'application/vnd.vella.signature-policy.v3+json';
...
function requireRuntime() {
  ...
  if (major !== 24 || minor < 20) throw new Error('E_PQ_RUNTIME: requires qualified Node 24.20+ within 24.x');
}
...
function verify(input, publicKeys, requiredSuite) {
  try {
    ...
    return {ok: true, format: KIND, suite: SUITE, authenticated: record,
      errors: [], warnings: ['Signature validity alone does not establish execution authority or preservation assurance']};
  } catch (error) {
