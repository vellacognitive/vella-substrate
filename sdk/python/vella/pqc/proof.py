...
"""Opt-in v3 hybrid proof implementation."""
import base64
import hashlib
import re
import sys
...
KIND = 'vella_proof_bundle_v3'
TYPE = 'application/vnd.vella.authorization.v3+json'
CONTEXT = 'application/vnd.vella.signature-policy.v3+json'
...
def require_runtime() -> tuple[type[MLDSA65PrivateKey], type[MLDSA65PublicKey]]:
    import cryptography
...
def private_key(algorithm: str, value: Any) -> ec.EllipticCurvePrivateKey | MLDSA65PrivateKey:
    ...
    return cast("ec.EllipticCurvePrivateKey | MLDSA65PrivateKey", key)
...
def pae(kind: str, payload: bytes) -> bytes:
    return f'DSSEv1 {len(kind.encode())} {kind} {len(payload)} '.encode() + payload


def message(payload: bytes, keys: dict[str, str]) -> bytes:
...
def sign(record: dict[str, Any], private_keys: object, required_suite: str) -> dict[str, Any]:
    ...
    supplied = cast(dict[str, Any], private_keys)
    loaded = {alg: private_key(alg, supplied[alg]) for alg in ALGORITHMS}
    ...
    for alg, key in loaded.items():
        if alg == 'ml-dsa-65':
            signature = cast('MLDSA65PrivateKey', key).sign(data)
        else:
            r, s = decode_dss_signature(cast(ec.EllipticCurvePrivateKey, key).sign(data, ec.ECDSA(hashes.SHA256())))
...
def verify(input_value: Any, public_keys: Any, required_suite: str) -> dict[str, Any]:
    try:
        required(required_suite)
        ...
        for alg in ALGORITHMS:
            ...
            sig = signatures[alg]
            if alg == 'ml-dsa-65':
                cast("MLDSA65PublicKey", key).verify(sig, data)
            else:
                der = encode_dss_signature(int.from_bytes(sig[:32], 'big'), int.from_bytes(sig[32:], 'big'))
                cast(ec.EllipticCurvePublicKey, key).verify(der, data, ec.ECDSA(hashes.SHA256()))
        return {'ok': True, 'format': KIND, 'suite': SUITE, 'authenticated': record, 'errors': [],
                    'warnings': ['Signature validity alone does not establish execution authority or preservation assurance']}
...
def generate_keys(required_suite: str) -> dict[str, Any]:
    required(required_suite)
    pq_private, _ = require_runtime()
    private_keys, public_keys = {}, {}
    for alg in ALGORITHMS:
        key = pq_private.generate() if alg == 'ml-dsa-65' else ec.generate_private_key(ec.SECP256R1())
        private_keys[alg] = {'format': 'raw-seed', 'value': base64.b64encode(cast("MLDSA65PrivateKey", key).private_bytes_raw()).decode()} if alg == 'ml-dsa-65' else key.private_bytes(serialization.Encoding.PEM, serialization.PrivateFormat.PKCS8, serialization.NoEncryption()).decode()
